...
def migration_test_url(source: str) -> URL:
    url = make_url(source)
    return url.set(database=f"{url.database}_analytics_migration_test")
        ...
        async with engine.connect() as connection:
            ...
            found = set(
                await connection.scalars(
                    text(
                        "SELECT indexname FROM pg_indexes WHERE indexname IN :indexes"
                    ).bindparams(bindparam("indexes", expanding=True)),
                    {"indexes": ANALYTICS_INDEXES},
                )
            )
            assert found == (set(ANALYTICS_INDEXES) if upgraded else set())
    finally:
...
def main() -> int:
    ...
    if app_url.host not in {"127.0.0.1", "localhost", "::1"}:
        raise RuntimeError("The analytics migration test accepts only local PostgreSQL")
    ...
    print(
        f"Preparing isolated analytics migration database {database_name!r}", flush=True
    )
    ...
    try:
        ...
        print(
            f"Removed isolated analytics migration database {database_name!r}",
            flush=True,
        )
    print("Analytics migration preservation test passed", flush=True)
    return 0
